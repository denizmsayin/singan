...
# set this module level variable to determine the device to use for inception
INCEPTION_DEVICE = 'cpu'


def _load_inception_block1():
    """ Load and return the inception network's first block (64 filters). """
    inception = inception_v3(pretrained=True).to(INCEPTION_DEVICE)  # need pre-trained weights for FID
...
class SIFIDInception(torch.nn.Module):
    """
    ...
    def __init__(self):
        super().__init__()
        self.inception = _load_inception_block1()
...
def get_activation_features(image):
    ...
    # lazy load the inception model
    if not hasattr(get_activation_features, 'model'):
        get_activation_features.model = SIFIDInception()
    ...
    features = get_activation_features.model(image).detach().cpu().numpy()
