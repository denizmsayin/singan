...
def get_activation_features(image):
    """
    ...
    Args:
        image: tensor with dimensions (1, C, H, W)  like output of load_image(). Pixels values must be in (0, 1)

    Returns:
        features (n, 64) numpy array
    """
    if get_activation_features.model is None:
        get_activation_features.model = InceptionV3(output_blocks=[0]).eval()

    features = get_activation_features.model(image)
...
get_activation_features.model = None
...
def get_mu_sigma(features):
    ...
    Args:
        features: numpy array of image features, like output of get_activation_features()
    ...
    Returns:
        ...
        sigma: numpy array, std of the gaussian distribution ((64x64) if the input features is
               output of get_activation_features())
    """
...
def calculate_sifid(img1, img2):
    ...
    Pixel values of the input images are mapped to (0, 1). Since SIFID is symmetric, order of the input images does not
    matter.
    Args:
        img1, img2: torch tensor (1, C, H, W). Pixel values must be in (-1, 1), these values are later mapped to (0, 1).
    Returns:
        sifid: numpy.float64, SIFID's of the two input images
    """
    # images are between (-1, 1), map them to (0, 1) as required by the inception network
    img1 = ((img1 + 1) / 2)
    img2 = ((img2 + 1) / 2)

    img1_features = get_activation_features(img1)
    img2_features = get_activation_features(img2)
    mu1, sigma1 = get_mu_sigma(img1_features)
    mu2, sigma2 = get_mu_sigma(img2_features)
    sifid = calculate_FID(mu1, sigma1, mu2, sigma2)
    """
    print("img1_features:", img1_features)
    print("mu1, sigma1:", mu1, sigma1)
    print("img2_features:", img2_features)
    print("mu2, sigma2:", mu2, sigma1)
    """
